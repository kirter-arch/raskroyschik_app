...
def calculator_page(db: Session):
    ...
    if st.button('Рассчитать и сохранить'):
        ...
        # Требуем единственный вид пленки для корректной цены
        ...
        # Разворачиваем по количеству и валидируем размеры
        parts_for_packing = []
        ...
        # Подсчет упакованных прямоугольников
        placed_count = sum(len(abin) for abin in packer)
        ...
        # Первый бин и фактическая использованная длина
        abin = packer[0]
        used_length_cm = 0
        for rect in abin:
        ...
        st.write(f"Общая площадь всех створок (запрошено): {total_area_requested_m2:.2f} м²")
        st.write(f"Площадь упакованных створок (факт): {placed_area_m2:.2f} м²")
